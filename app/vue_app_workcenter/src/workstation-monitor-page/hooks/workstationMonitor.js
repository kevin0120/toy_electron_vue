...
export function useCurveDataWS(workcenter_code) {
        ...
        nextTick(() => {
            if (e.wheelDelta || e.detail) {
                if (e.wheelDelta > 0 || e.detail < 0) {
                    //当鼠标滚轮向上滚动时
                    window.scrollTo({
                        top: domRef.value?.getBoundingClientRect().top || 0,
                        behavior: 'smooth'
                    });
                }
                if (e.wheelDelta < 0 || e.detail > 0) {
                    //当鼠标滚轮向下滚动时
                    window.scrollTo({
                        top: domRef.value?.scrollHeight || 0,
                        behavior: 'smooth'
                    });
                }
            }
    ...
    const getCharts = (entity_id, index, markLine, markPointData) => {
        try {
            chartList.value[index] = [
                seriesLineFormatter(index, curveDataList.value[index].map((arr) => ({
                    value: arr,
                ...
                })), entity_id, analysisCurveResult?.value?.tightening_result || 'default', true, markLine, markPointData)
            ];
        }
        catch (e) {
            toast.add({
    ...
    watch(probaResult, (probaResult) => {
        getCharts(analysisCurveResult.value?.entity_id || '', activeTabIdx.value, getMarkLine(torqueRange.value, angleRange.value, activeTabIdx.value), getMarkPointData(probaResult, curveDataList.value[activeTabIdx.value]));
    });
    ...
    watch(() => analysisCurveResult.value, (value) => {
        if (value?.entity_id) {
            getCharts(value?.entity_id, activeTabIdx.value, getMarkLine(torqueRange.value, angleRange.value, activeTabIdx.value));
            openModal();
        }
    });
