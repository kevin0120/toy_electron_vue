  ...
  <div ref="container" :class="containerClass" @click="onContainerClick">
    ...
    <Portal :append-to="appendTo">
      ...
      >
        ...
        >
          <DraggList :list="modelValue" icon="pi pi-cog" @onChangeList="onChangelist"></DraggList>
        </div>
      </transition>
    </Portal>
  </div>
...
<script>
  ...
  import DraggList from '/@/components/DraggList/DraggList.vue';
  ...
  export default {
    ...
    computed: {
      ...
      dropdownIconClass() {
        return ['p-multiselect-trigger-icon', this.loading ? this.loadingIcon : this.icon];
      },
      panelStyleClass() {
        return [
        ...
        return this.selectAll !== null
          ...
          : ObjectUtils.isNotEmpty(this.visibleOptions) &&
              this.visibleOptions
                ...
                .every((option) => this.isOptionGroup(option) || this.isValidSelectedOption(option));
      },
      hasSelectedOption() {
      ...
      toggleAllAriaLabel() {
        return this.$primevue.config.locale.aria
          ? this.$primevue.config.locale.aria[this.allSelected ? 'selectAll' : 'unselectAll']
          : undefined;
      },
      closeAriaLabel() {
    ...
    methods: {
      ...
      onOptionSelect(event, option, index = -1, isFocus = false) {
        ...
        if (selected) {
          value = this.modelValue.filter(
            (val) => !ObjectUtils.equals(val, this.getOptionValue(option), this.equalityKey)
          );
      ...
      getLabelByValue(value) {
        ...
        const matchedOption = options.find(
          (option) =>
            !this.isOptionGroup(option) && ObjectUtils.equals(this.getOptionValue(option), value, this.equalityKey)
        );
      ...
      onToggleAll(event) {
        if (this.selectAll !== null) {
          ...
          const value = this.allSelected
            ? []
            : this.visibleOptions
                .filter((option) => !this.isOptionDisabled(option) && !this.isOptionGroup(option))
                .map((option) => this.getOptionValue(option));
      ...
      removeOption(event, optionValue) {
        const value = this.modelValue.filter((val) => !ObjectUtils.equals(val, optionValue, this.equalityKey));
      ...
      findPrevOptionIndex(index) {
        const matchedOptionIndex =
          index > 0
            ? ObjectUtils.findLastIndex(this.visibleOptions.slice(0, index), (option) => this.isValidOption(option))
            : -1;
