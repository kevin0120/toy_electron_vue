<template>
  <div class="color-box">
    ...
    ></div>
    <Transition>
      <div v-show="isShowColorList" ref="colorWheel" role="colorWheel" class="color-list" @click="selectColor">
        <section
...
<script lang="ts" setup>
  ...
  const tooltip = ref<null | HTMLDivElement>(null);
  ...
  const onInit = () => {
    nextTick(() => {
      if (divRef.value && tooltip.value) {
        createPopper(divRef.value, tooltip.value, {
          placement: 'right',
          modifiers: [
            {
              name: 'offset',
