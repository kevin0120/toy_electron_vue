...
import type { chartLabel, tCurveInfo } from '/@/api/types/curves';
...
export function useCurveData(curveId: string) {
  const toast = useToast();
  const colors = ref(randomHexColorList(3));
  const activeTabIdx = ref(0);
  const analysisCurveResult = ref<tCurveInfo>(initCurveInfo);
  ...
  const getCharts = async (index: number, markLine: any) => {
    try {
      ...
      chartList.value[index] = [
        seriesLineFormatter(
          index,
          res.data[0].curve_data.map((arr: [number, number]) => ({
            value: arr,
