<template>
  <AppLayout :home="{ icon: 'pi pi-home', to: '/' }" :model="[{ label: '曲线分析', disabled: true }]">
    <template #AppBody>
      <div class="grid h-full w-full bg-white p-2" style="min-height: 850px">
        ...
        <div class="col-12 md:col-12 lg:col-3 xl:col-3 h-calc-10rem">
          <div class="p-card h-full w-full p-4" style="border-radius: 16px">
            <div class="grid w-full" style="height: 90%; overflow: auto">
              <CurveInfoList :curve-info="analysisCurveResult" />
            </div>
            <div class="flex justify-content-around align-items-center" style="height: 10%">
              <Button
                style="width: 48%"
                label="曲线比较"
                icon="pi pi-chart-line"
                @click="routerToCurveComparison(analysisCurveResult.bolt_name, curveId)"
              />
              <Button style="width: 48%" label="下载曲线" icon="pi pi-download" @click="downloadCurveFile" />
            </div>
          </div>
        </div>
      </div>
...
<script lang="ts" setup>
  ...
  import { useCurveData } from '/@/views/curve-analysis-page/hooks/curveAnalysis';
  import { usePageRouter } from '/@/hooks/modules/usePageRouter';
  import { echartfontSize } from '/@/utils/autoAdaptive';
  const { router, routerToCurveComparison } = usePageRouter();
  const curveId = String(router.currentRoute.value.params.entity_id || '');
  const { colors, analysisCurveResult, activeTabIdx, chartLabel, chartList, downloadCurveFile } = useCurveData(curveId);
  const newFontSize: string = echartfontSize(0.16) + 'px';
</script>
